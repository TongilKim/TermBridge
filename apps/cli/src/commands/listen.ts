...
export function createListenCommand(): Command {
  ...
  command
    .description('Listen for session requests from mobile app')
    ...
    .action(async (options: ListenOptions) => {
      ...
      const spinner = new Spinner('Starting listener...');
      ...
      try {
        ...
        if (!connected) {
          logger.error('Failed to connect to realtime. Check your network connection.');
          process.exit(1);
        }

        logger.info('');
        logger.info('✓ Listening for session requests');
          ...
          if (cmd.type === 'start-session') {
            ...
            try {
              daemon = new Daemon({
                ...
                cwd: process.cwd(),
                hybrid: false, // No local terminal input in listen mode
              });
              ...
              daemon.on('started', async ({ session }) => {
                ...
                await machineClient?.broadcastSessionStarted(session.id, process.cwd());
              });
